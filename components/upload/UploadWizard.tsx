...
function Stepper({ phase }: { phase: string }) {
  ...
  const normalized =
    phase === "initializing" ? "uploading" : phase === "error" || phase === "canceled" ? "uploading" : phase
...
export default function UploadWizard() {
  ...
  const isBusy = status === "validating" || status === "initializing" || status === "uploading" || status === "finalizing"
  const canResume = (status === "error" || status === "canceled") && !!sessionId && !!chunks
  ...
  useEffect(() => {
    ...
    if (status === "validating") toast.loading("Validating file…", { id: "upload" })
    if (status === "initializing") toast.loading("Starting upload…", { id: "upload" })
    if (status === "uploading") toast.loading("Uploading…", { id: "upload" })
    if (status === "finalizing") toast.loading("Finalizing…", { id: "upload" })
    if (status === "done") toast.success("Upload complete. Preview is ready.", { id: "upload" })
    if (status === "canceled") toast("Upload canceled.", { id: "upload" })
    if (status === "error") toast.error("Upload failed. See details below.", { id: "upload" })
  ...
  const stageDescription = useMemo(() => {
    ...
    if (status === "validating") return "Checking that the file looks like a CSV and isn’t empty."
    if (status === "initializing") return "Creating a secure upload session."
    if (status === "uploading") return "Sending your file in small parts so it works under request size limits."
    if (status === "finalizing") return "Assembling uploaded parts into a single CSV and preparing the preview."
    if (status === "done") return "Upload is fully complete. You can now review the preview."
    if (status === "canceled") return "Upload was stopped. You can resume or restart."
    if (status === "error") return "Some parts may have failed. You can retry the missing parts or restart."
    return ""
  }, [status])
    ...
    <div>
        ...
        <div className={styles.body}>
          <div style={{ marginBottom: 16 }}>
            ...
            <p className={styles.sectionSubtitle}>
              Large files are sent in parts (chunked) to work under request size limits.
            </p>
          </div>
          ...
          {!hasFile && !isDone ? (
            <div
              ...
              title={isBusy ? "Upload in progress" : "Click to upload or drag and drop"}
            >
          ...
          {hasFile && !isDone ? (
            <div className={styles.fileSelectedCard}>
                ...
                <div className={styles.fileMeta}>
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                    <div style={{ minWidth: 0 }}>
                      <p className={styles.fileName}>{file?.name ?? "Selected file"}</p>
                  ...
                  <div className={styles.progressTopRow}>
                    ...
                    <div className={styles.progressPct}>{percent(progress)}</div>
                  </div>
                  <div className={styles.progressTrack}>
                    <div className={styles.progressBar} style={{ width: percent(progress) }} />
                  </div>
              ...
              {error ? (
                <div style={{ marginTop: 12, borderRadius: 12, border: "1px solid rgba(176,0,32,0.18)", background: "rgba(176,0,32,0.06)", padding: 12 }}>
                  <div style={{ fontWeight: 800, color: "#b00020" }}>Upload failed</div>
                  <div style={{ marginTop: 6, color: "#b00020", fontSize: 13, lineHeight: 1.4 }}>
                    <div><b>Error:</b> {error}</div>
                    {errorHint ? <div style={{ marginTop: 4, color: "#8a0019" }}><b>What you can do:</b> {errorHint}</div> : null}
                    {failedChunkCount > 0 && canResume ? (
                      <div style={{ marginTop: 6, color: "#8a0019" }}>
                        Tip: Click “Resume Missing Parts” to retry only the failed parts.
                      </div>
                    ) : null}
                  </div>
                </div>
          ...
          <InfoBox>
            <div className={styles.infoRow}>
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ fontWeight: 800, color: "#1e3a8a", marginBottom: 4, fontSize: 13 }}>How it works</div>
                <ul style={{ margin: 0, paddingLeft: 18, color: "#1e40af", fontSize: 13, lineHeight: 1.6 }}>
                  <li>Files are split into smaller chunks to handle large uploads efficiently.</li>
                  <li>You can cancel and resume missing parts (within this page session).</li>
                  <li>After upload completes, you’ll see a preview of your dataset.</li>
                </ul>
              </div>
            </div>
          </InfoBox>
      ...
      {isDone && showGoToPreview ? (
        <div className={styles.successCard}>
          ...
          <p className={styles.successSub}>Your file has been successfully uploaded and is ready for preview.</p>
