...
export default function DataPreviewTable() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      setError(null)
      setData(null)
      const res = await fetch(`/api/upload/finalize?sessionId=${encodeURIComponent(effectiveSessionId)}`, { method: "GET" })
      if (!res.ok) {
  ...
  if (!effectiveSessionId) return <div className={styles.muted}>Missing sessionId. Upload first, then click “Go to preview”.</div>
  ...
  const filteredColumns = columns.filter((c) => c.toLowerCase().includes(columnQuery.trim().toLowerCase()))
      ...
      <div className={styles.card}>
        <div className={styles.headerRow}>
          ...
          <div className={`${styles.muted}`} style={{ fontSize: 12 }}>
            Session <span className={styles.mono}>{effectiveSessionId.slice(0, 8)}…</span>
          </div>
        </div>
        ...
        {issues.length > 0 ? (
          <div className={styles.issuesList}>
            {issues.map((iss, idx) => (
              <div key={idx} className={`${styles.issue} ${issueClass(iss.severity)}`}>
          ...
          <div style={{ marginTop: 10, fontSize: 12, color: "#146c43" }}>No obvious issues found in the preview.</div>
        )}
      </div>
      ...
      <div className={styles.twoCol}>
        <div className={styles.card}>
          ...
          <div className={styles.btnRow}>
            <button onClick={() => setSelectedColumns(new Set(columns))} className={styles.btn}>Show all</button>
            <button onClick={() => setSelectedColumns(new Set(columns.slice(0, Math.min(12, columns.length))))} className={styles.btn}>Show first 12</button>
            <button onClick={() => setSelectedColumns(new Set())} className={styles.btn}>Clear</button>
          </div>
          ...
          <div className={styles.colList}>
            <div style={{ display: "grid" }}>
                  ...
                  >
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div className={styles.colMeta}>
                        ...
                        {examples.length > 0 ? <span> • e.g. “{examples[0]}”</span> : null}
                      </div>
                    </div>
                  </label>
                )
              ...
              {filteredColumns.length === 0 ? (
                <div style={{ padding: 10, fontSize: 12, color: "#666" }}>No columns match that filter.</div>
              ) : null}
            </div>
          </div>

          <div style={{ marginTop: 10, fontSize: 12, color: "#666" }}>
            Tip: For wide files, start with a few key columns, then add more if needed.
          </div>
        </div>
        ...
        <div className={styles.card}>
          <div className={styles.headerRow}>
            ...
            <div className={styles.muted} style={{ fontSize: 12 }}>Showing {rows.length} rows</div>
          </div>
                    ...
                    {visibleColumns.map((c) => (
                      <th key={c} className={styles.th}>
                        <div className={styles.thInner}>
                          <span>{c}</span>
                          <span className={styles.colType}>{types[c] ?? "unknown"}</span>
                        </div>
                      </th>
                    ))}
                  </tr>
